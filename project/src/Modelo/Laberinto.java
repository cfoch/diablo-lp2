...
public class Laberinto {
	...
	public void pintarArea(Integer x, Integer y, Integer x1, Integer y1,
	                       Integer tipo) {
		Integer xI, xF, yI, yF;
		xI = x;
		yI = y;
	...
	public void anadirElementos(int n) {
		...
		int numEnemigos = 100, numArmas = 3, numArmaduras = 1,
		    numPociones = 2;
		for (i = 0; i < numEnemigos; i++) {
			Celda c = dentro.get(index.get(i));
			enemigos.add(new Enemigo(c.getX(), c.getY(), 0, i,
			                         "Tipo " + i));
			cambiarTipo(c.getX(), c.getY(), 5);
		}
		for (; i < numEnemigos + numArmas; i++) {
			Celda c = dentro.get(index.get(i));
			artefactos.add(new Arma(c.getX(), c.getY(),
			                        i - numEnemigos,
			                        "Tipo " + (i - numEnemigos),
			                        Myrandom(5, 10),
			                        Myrandom(10, 15)));
		...
		for (; i < numEnemigos + numArmas + numArmaduras; i++) {
			Celda c = dentro.get(index.get(i));
			artefactos.add(new Armadura(c.getX(), c.getY(),
			                            Myrandom(20, 30),
			                            i -
			                            (numEnemigos + numArmas),
			                            "Tipo " +
			                            (i -
			                             (numEnemigos +
                        numArmas))));
		...
		for (; i < numEnemigos + numArmas + numArmaduras + numPociones;
		     i++) {
			Celda c = dentro.get(index.get(i));
			artefactos.add(new PocionCuracion(c.getX(), c.getY(),
			                                  Myrandom(15, 25),
			                                  i -
			                                  (numEnemigos +
			                                   numArmas +
	...
	public Integer comprobarTipo(Integer x, Integer y) {
		if (getCelda(x,
		    ...
		    getCelda(x,
		             y).getTipo() == 7 ||
		    getCelda(x, y).getTipo() == 8) {                                                                                                                             // SIN EL ENEMIGO AHORA CON EL
			return -1;
		}
	...
	public void moverEnemigos(int av_X,int av_Y) {
		...
		for (int i = 0; i < enemigos.size(); i++) {
			...
			if (index.get(0) == 1) {
				...
				respuesta = comprobarTipo(enemigos.get(
								  i).getX(),
				                          enemigos.get(
								  i).getY() +
                                1);
				if ((enemigos.get(i).getX()==av_X &&
				     (enemigos.get(i).getY() + 1)==
				     av_Y)) respuesta =-1;
				if (respuesta != -1) {
					enemigos.get(i).setY((Integer) (enemigos
					                                .get(i).
					                                getY() +
					                                1));
				}
				...
				respuesta = comprobarTipo(enemigos.get(
								  i).getX() + 1,
				                          enemigos.get(i).getY());
				if (((enemigos.get(i).getX()+1)==av_X &&
				     (enemigos.get(i).getY())==
				     av_Y)) respuesta =-1;
				if (respuesta != -1) {
					enemigos.get(i).setX((Integer) (enemigos
					                                .get(i).
					                                getX() +
					                                1));
				}
